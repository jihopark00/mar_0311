...
class DeterministicDistribution:
    """Simple deterministic distribution wrapper for non-VAE models."""
    def __init__(self, x):
        self.x = x
...
class PatchifyVAE(nn.Module):
    """
    Simple pixel-space VAE for MAR training.

    When patch_size=1 (vae_stride=1 in config), this acts as identity transform
    with optional normalization. The actual patchification is done by MAR model.

    Args:
        patch_size: Spatial stride (1 = identity, >1 = average pooling)
        imagenet_normalize: Whether to normalize with ImageNet stats
    """
...
class AutoencoderKL_original(nn.Module):
    def __init__(self, embed_dim, ch_mult, use_variational=True, ckpt_path=None):
        super().__init__()
        self.encoder = Encoder(ch_mult=ch_mult, z_channels=embed_dim)
        self.decoder = Decoder(ch_mult=ch_mult, z_channels=embed_dim)
