"""
Main training script for xAR models with configurable VAE.

All VAE configuration (class, arguments, checkpoint path) is specified
in the YAML config file, allowing seamless swapping of different VAE
architectures without code changes.

Usage:
    python main_mar.py --config configs/marflow_ssl_latent.yaml

Example VAE section in config YAML:
    vae:
      class: AutoencoderKL
      kwargs:
        embed_dim: 16
...
def get_args_parser():
    parser = argparse.ArgumentParser('xAR Training', add_help=False)

    # Config
    parser.add_argument('--config', required=True, type=str,
                        help='Path to YAML config (must include a "vae" section)')
...
def main(args):
    ...
    # ------------------------------------------------------------------
    # VAE config from YAML
    # ------------------------------------------------------------------
    # vae_cfg = cfg.get('vae')
    # if vae_cfg is None:
    #     raise ValueError(
    #         "Config YAML must include a 'vae' section. Example:\n"
    #         "vae:\n"
    #         "  class: AutoencoderKL\n"
    #         "    latent_mean: 0.0\n"
    #         "    latent_std: 4.3011\n"
    ...
    # latent_mean / latent_std are stored on the VAE object via kwargs
    ...
    if global_rank == 0 and args.wandb_project:
        ...
        wandb_run = wandb.init(
            ...
            config={**vars(args), 'model': model_name, **model_config, 'vae': vae_name, 'vae_config': vae_config},
        )
    ...
    # VAE from config
    # ------------------------------------------------------------------
    # vae_cls_name = vae_cfg['class']
    # vae_kwargs = vae_cfg.get('kwargs', {})
    ...
    vae = vae_cls(**vae_config).cuda().eval()
    for param in vae.parameters():
        param.requires_grad = False

    print(f"VAE: {vae_name}")
    print(f"VAE kwargs: {vae_config}")
    ...
    if global_rank == 0:
        ...
        # GT grid
        gt_grid = make_grid(sample_images, nrow=nrow, normalize=False, padding=2)
        gt_grid_path = os.path.join(run_dir, "gt_train_images_grid.png")
        save_image(gt_grid, gt_grid_path)
        print(f"Saved GT training image grid ({sample_images.size(0)} images) to: {gt_grid_path}")
        ...
        print(f"Saved VAE reconstruction grid to: {recon_grid_path}")

        if wandb_run is not None:
            import wandb
            wandb_run.log({
                "gt_train_images": wandb.Image(gt_grid_path),
                "vae_recon_images": wandb.Image(recon_grid_path),
            }, step=0)
        del sample_iter, sample_images, sample_labels, recon_images
