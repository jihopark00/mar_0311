...
class MARSSL(nn.Module):
    ...
    def sslenc_forward(self, x, masks, class_embedding): # x is image! (patchified)
        x = self.unpatchify(x) # unpatchify to image space for SSL encoder
        if self.img_size_ssl != self.img_size:
            x = resize_image_per_patch(x, self.patch_size, self.sslenc.patch_size, interpolate_mode='nearest')
        bsz = x.size(0)
