...
def _dino_block_forward_with_cond(self, x, cond_num=0):
    """Override of dinov2 block forward adding a *separate* LayerScale path for
    the first `cond_num` tokens.

    The first `cond_num` tokens are the MAR buffer/condition tokens (derived
    from class embedding). They are scaled by `ls1_cond` / `ls2_cond` instead
    of the shared `ls1` / `ls2`, so their residual contribution can be learned
    independently from the image-token stream (which is what standard dinov2
    LayerScale was calibrated for).

    Mirrors the plain dinov2 Block path (norm → attn/mlp → ls → drop_path →
    residual). Does NOT cover the NestedTensorBlock / stochastic-depth
    branches — those aren't used with our inputs.
    """
    # --- attention residual -------------------------------------------------
    attn_out = self.attn(self.norm1(x))
    if cond_num > 0 and hasattr(self, 'ls1_cond'):
        attn_cond = self.ls1_cond(attn_out[:, :cond_num])
        attn_rest = self.ls1(attn_out[:, cond_num:])
        attn_out = torch.cat([attn_cond, attn_rest], dim=1)
    else:
    ...
    if cond_num > 0 and hasattr(self, 'ls2_cond'):
        mlp_cond = self.ls2_cond(mlp_out[:, :cond_num])
        mlp_rest = self.ls2(mlp_out[:, cond_num:])
        mlp_out = torch.cat([mlp_cond, mlp_rest], dim=1)
    else:
        mlp_out = self.ls2(mlp_out)
    # dinov2 uses drop_path2 if present, otherwise reuses drop_path1.
    dp2 = getattr(self, 'drop_path2', self.drop_path1)
    x = x + dp2(mlp_out)

    return x


class MAR_DINO_0419_LayerScale(nn.Module):
    """ MAR with dinov2 transformer blocks injected between the MAR encoder and decoder.

    Variant of MAR_DINO_0419 that adds a *separate* learnable LayerScale
    (`ls1_cond`, `ls2_cond`) for the MAR buffer (condition) tokens inside each
    dinov2 block. Motivation: the condition signal is injected only at the
    first transformer layer and can weaken as the shared LayerScale was
    calibrated on image tokens; giving the condition stream its own γ lets
    the model preserve/amplify the conditioning signal across depth.

    The dinov2 source is left untouched — the block forward is monkey-patched
    here to route the first `cond_num` tokens through `ls*_cond`.
    """
    def __init__(self, img_size=256, vae_stride=16, patch_size=1,
        ...
        # --------------------------------------------------------------------------
        # Per-block *condition* LayerScale + forward override.
        # `ls1_cond` / `ls2_cond` are deep copies of the block's existing
        # `ls1` / `ls2` so they inherit the pretrained γ (when applicable)
        ...
        # --------------------------------------------------------------------------
        # Optional external pos_embed for the dinov2 stage: replaces per-forward
        # interpolate_pos_encoding with a learnable param sized exactly for our
        # sequence (cls + image tokens). Initialized from interpolate_pos_encoding.
        ...
        # Also strip ls*_cond so the cond stream stays consistent with the
        # shared stream — otherwise "no LayerScale" would silently only apply
        # to image tokens.
    ...
    def _apply_dino_tuning(self, freeze_dino=None, freeze_dino_blocks=None,
        ...
        - lora_config keys:
            ...
            trainable_modules (list[str], default []) — block attr names
                (e.g. 'norm1','norm2','ls1','ls2','ls1_cond','ls2_cond')
                kept trainable after the per-block freeze.
    ...
    def initialize_weights(self):
        ...
        if not self.dinov2_pretrained:
            ...
            # LayerScale γ: scratch 안정성 위해 1.0 → 1e-5.
            # ls*_cond is reset to the same init so the cond stream starts
            # identically to the shared stream; they diverge during training.
            for blk in self.dinov2_backbone.blocks:
                for ls in (blk.ls1, blk.ls2, blk.ls1_cond, blk.ls2_cond):
                    if hasattr(ls, 'gamma'):
                        nn.init.constant_(ls.gamma, 1e-5)

        # Init external dinov2 pos_embed from interpolate_pos_encoding so it
        # matches the pretrained pos grid at our seq_len (runs after any
        # dinov2.pos_embed re-init above).
    ...
    def forward_mae_decoder(self, x, mask):
        ...
        # Apply pos_embed: either the learnable external param (sized exactly
        # for cls + image tokens) or dinov2's on-the-fly interpolate_pos_encoding.
        if self.dinov2_external_pos_embed:
            image_tokens = image_tokens + self.dinov2_external_pos_embed_param.to(image_tokens.dtype)
        else:
            dpatch = self.dinov2_backbone.patch_size
            w = self.seq_w * dpatch
            h = self.seq_h * dpatch
        ...
        # 5. Dinov2 transformer blocks (optional grad checkpointing), with a
        #    per-block `cond_num=buf` so the first `buf` tokens get their own
        #    LayerScale (ls*_cond) on both residual paths.
        if self.grad_checkpointing and self.training and not torch.jit.is_scripting():
            for blk in self.dinov2_backbone.blocks:
                x = checkpoint(blk, x, buf)
        else:
            for blk in self.dinov2_backbone.blocks:
                x = blk(x, buf)
